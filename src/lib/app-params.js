...
const getAppParamValue = (paramName, { defaultValue = undefined, removeFromUrl = false } = {}) => {
	...
	const storageKey = `standalone_${toSnakeCase(paramName)}`;
...
const getAppParams = () => {
	if (getAppParamValue("clear_access_token") === 'true') {
		storage.removeItem('standalone_access_token');
		storage.removeItem('token');
	}
	return {
		appId: getAppParamValue("app_id", { defaultValue: import.meta.env.VITE_APP_ID || "standalone" }),
